...
const validateSchema = async (instance, $ref) => {
  ...
  const relatedSchemes = [
    ...
    'ccl-get-dcc-admission-check-scenarios.json',
    'ccl-get-dcc-wallet-info.json',
    'ccl-get-status-tab-notice.json',
...
export default {
  ...
  api: {
    getDccAdmissionCheckScenarios: input => {
      return evaluateFunction('getDccAdmissionCheckScenarios', input)
    },
    getDccWalletInfo: input => {
      return evaluateFunction('getDccWalletInfo', input)
    },
    getStatusTabNotice: input => {
      return evaluateFunction('getStatusTabNotice', input)
    }
  },
  schema: {
    ...
    functions: {
      getDccAdmissionCheckScenarios: {
        input: {
          validate: instance => validateSchema(instance, 'https://ccl.coronawarn.app/functions/getDccAdmissionCheckScenarios#/$defs/input')
        },
        output: {
          validate: instance => validateSchema(instance, 'https://ccl.coronawarn.app/functions/getDccAdmissionCheckScenarios#/$defs/output')
        }
      },
      getDccWalletInfo: {
        input: {
      ...
      getStatusTabNotice: {
        input: {
          validate: instance => validateSchema(instance, 'https://ccl.coronawarn.app/functions/getStatusTabNotice#/$defs/input')
        },
        output: {
          validate: instance => validateSchema(instance, 'https://ccl.coronawarn.app/functions/getStatusTabNotice#/$defs/output')
        }
      },
